...
import org.protempa.AbstractDataSourceConstraintVisitor;
import org.protempa.DataSourceConstraint;
import org.protempa.PositionDataSourceConstraint;
...
public abstract class AbstractSQLGenerator implements SQLGenerator {
            ...
            for (ColumnSpec cs3 : constraintSpecs) {
                while (true) {
                    if (cs3.getJoin() != null) {
                        ...
                        if (csc != null) {
                            if (ColumnSpec.Constraint.EQUAL_TO.equals(csc)) {
                                ...
                                if (cvs.length > 1) {
                                    ...
                                    wherePart.append("=");
                                    appendValue(cvs[0].getValue(), wherePart);
                                }
                            } else if (ColumnSpec.Constraint.LIKE.equals(csc)) {
                                ...
                                for (int k = 0; k < cvs.length; k++) {
                                    ...
                                    appendValue(cvs[k].getValue(), wherePart);
                                    if (k + 1 < cvs.length) {
                                        wherePart.append(" or ");
                                    }
                                    ...
                                    appendValue(cvs[k].getValue(), selectPart);
                                    selectPart.append(" then ");
                                    selectPart.append("'");
                                    appendValue(cvs[k].getCode(), selectPart);
                                    selectPart.append("'");
                                }
                                if (cvs.length > 1) {
                                    wherePart.append(')');
                                }
                            } else if (ColumnSpec.Constraint.IN.equals(csc)) {
                                wherePart.append("a");
                                wherePart.append(referenceIndices.get(cs3));
                                wherePart.append(".");
                                wherePart.append(cs3.getColumn());
        ...
        if (finishTimeSpec != null) {
            while (true) {
                if (finishTimeSpec.getJoin() != null) {
                    ...
                    DataSourceConstraint dsc2 = dataSourceConstraints;
                    while (true) {
    ...
    private int processStartTimeSpecForWhereClause(
            ...
            StringBuilder wherePart,
            Map<ColumnSpec, Integer> referenceIndices) {
        ColumnSpec startTimeSpec = propositionSpec.getStartTimeSpec();
                ...
                if (startTimeSpec.getJoin() != null) {
                    ...
                    while (true) {
                        if (dsc2 instanceof PositionDataSourceConstraint) {
                            ...
                            wherePart.append("'}");
                            if (propositionSpec.getFinishTimeSpec() == null) {
                                wherePart.append(" and ");
                                wherePart.append('a');
                                wherePart.append(
                                    referenceIndices.get(startTimeSpec));
