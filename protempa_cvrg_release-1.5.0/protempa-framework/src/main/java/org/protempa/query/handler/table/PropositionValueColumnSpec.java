 ...
 * given two numbers <code>< 4</code> and <code>3</code>, we convert
...
public class PropositionValueColumnSpec extends AbstractTableColumnSpec {
	...
	public String[] columnValues(String key, Proposition proposition,
		...
		LOOP_PROPOSITIONS: for (Proposition prop : propositions) {
		...
		if (sumTotal != null) {
			if (this.type == Type.AVG) {
				return new String[] { NumberValue.getInstance(
						sumTotal.divide(BigDecimal.valueOf(count.longValue())))
						.getFormatted() };
			} else {
				return new String[] { NumberValue.getInstance(sumTotal)
						.getFormatted() };
			}
		} else if (this.type == Type.MEDIAN) {
			NumberValue median = medianValue(orderStats);
			if (median != null) {
				return new String[] { median.getFormatted() };
			} else {
