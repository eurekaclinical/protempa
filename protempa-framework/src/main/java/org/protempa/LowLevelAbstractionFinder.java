				...
				if ((foundValue = def.satisfiedBy(seg, algorithm)) != null) {
					...
					do {
						if (lastSeg != null
								...
								&& (HTI.execute(def, lastSeg, seg) || gf
										.execute(lastSeg, seg))) {
							lastSeg.resetState(seq, Math.min(lastSeg
									.getFirstIndex(), seg.getFirstIndex()),
									Math.max(lastSeg.getLastIndex(), seg
											.getLastIndex()));
						} else {
							if (lastSeg != null) {
								objAsserter
										.assertObject(AbstractParameterFactory
												.getFromAbstraction(id,
														lastSeg, prevFoundValue
																.getValue(),
														null));
							}
						...
						prevFoundValue = foundValue;
							...
							&& (foundValue = def
									.satisfiedBy(nextSeg, algorithm)) != null);
				}
			} while (advanceRow(def, seg, lastSeg, algorithm, minPatternLength,
					maxPatternLength) != null);
			if (lastSeg != null) {
				objAsserter.assertObject(AbstractParameterFactory
						.getFromAbstraction(id, lastSeg, prevFoundValue
								.getValue(), null));
				lastSeg = null;
			}
		}
