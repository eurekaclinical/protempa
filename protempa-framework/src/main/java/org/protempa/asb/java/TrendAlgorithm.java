...
public final class TrendAlgorithm extends AbstractAlgorithm {
	...
	public TrendAlgorithm(Algorithms algorithms, String id) {
		...
		setParameters(new AlgorithmParameter[] {
				new AlgorithmParameter("minThreshold", new ValueComparator[] {
						...
						ValueComparator.GREATER_THAN }, ValueFactory.NUMERICAL),
	...
	public Value compute(
		...
		ValueComparator minSlopeThresholdComparator = args
				.valueComp("minThreshold");
		Value maxSlopeThreshold = args.value("maxThreshold");
		ValueComparator maxSlopeThresholdComparator = args
				.valueComp("maxThreshold");
		...
		if ((minSlopeThresholdComparator != null && minSlopeThreshold != null)
			...
			if ((minSlopeThresholdComparator != null && !minSlopeThresholdComparator
					.getConsistent(slope.compare(minSlopeThreshold)))
					|| (maxSlopeThresholdComparator != null && !maxSlopeThresholdComparator
							.getConsistent(slope.compare(maxSlopeThreshold)))) {
				return null;
			}
		}
