...
public class Segment<T extends TemporalProposition> extends AbstractList<T>
	...
	public Segment(Segment<T> segment) {
		this(segment != null ? segment.ts : null, segment != null ? segment
				.getFirstIndex() : 0, segment != null ? segment.getLastIndex()
				: 0);
	}
	...
	private void calcMaxFinishParam() {
		if (maxFinishParam == null) {
			maxFinishParam = Collections.max(this,
					PropositionUtil.MAX_FINISH_COMPARATOR);
		}
	}
	...
	private Long maximumStart(
			Segment<T> segment) {
		return Collections.min(segment, MAX_START_COMP).getInterval()
				.getMaxStart();
	}
	...
	private Long minimumFinish(
			Segment<T> segment) {
		return Collections.max(segment, MIN_FINISH_COMP).getInterval()
				.getMinFinish();
	}
}
