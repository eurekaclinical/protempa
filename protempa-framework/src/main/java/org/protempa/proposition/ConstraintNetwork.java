 ...
 * @author Andrew Post
 */
public final class ConstraintNetwork implements Serializable {

	private static final long serialVersionUID = 1712682738676203844L;

	private final String timeZero = "0";

	private final List<Interval> intervals;

	private final DirectedGraph directedGraph;

	private transient Weight calcMinDuration;

	private transient Weight calcMaxDuration;

	private transient Weight calcMinFinish;

	private transient Weight calcMaxFinish;

	private transient Weight calcMinStart;

	private transient Weight calcMaxStart;

	private transient Map<?, Weight> shortestDistancesFromTimeZeroSource;

	private transient Map<?, Weight> shortestDistancesFromTimeZeroDestination;
	 ...
	 */
	public ConstraintNetwork(int initialCapacity) {
	 ...
	 *         otherwise.
	 */
	public boolean removeRelation(Interval i1, Interval i2) {
	 ...
	 */
	public Weight getMinimumStart() {
		if (calcMinStart == null) {
			// Find the shortest distance from a start to time zero.
			Weight result = Weight.NEG_INFINITY;
			if (shortestDistancesFromTimeZeroDestination == null) {
				shortestDistancesFromTimeZeroDestination = BellmanFord
						.calcShortestDistances(timeZero, directedGraph,
			...
			for (int i = 0, n = intervals.size(); i < n; i++) {
				...
				result = Weight.max(result,
						(Weight) shortestDistancesFromTimeZeroDestination
								.get(start));
			}
	...
	public Weight getMaximumStart() {
		if (calcMaxStart == null) {
			...
			for (int i = 0, n = intervals.size(); i < n; i++) {
				...
				result = Weight
						.min(result,
								(Weight) shortestDistancesFromTimeZeroSource
										.get(start));
			}
			calcMaxStart = result;
		}
	 ...
	 * @return a <code>Weight</code> object.
	 */
	public Weight getMinimumFinish() {
	 ...
	 * @return a <code>Weight</code> object.
	 */
	public Weight getMaximumFinish() {
	 ...
	 * @return a <code>Weight</code> object.
	 */
	public Weight getMaximumDuration() {
	 ...
	 * @return a <code>Weight</code> object.
	 */
	public Weight getMinimumDuration() {
		if (calcMinDuration == null) {
			Weight min = Weight.POS_INFINITY;
			for (int i = 0, n = intervals.size(); i < n; i++) {
				Object finish = intervals.get(i).getFinish();
