...
public class AbsoluteTimeUnit extends AbstractTimeUnit {
	...
	static final double avgDaysInMonth = 30.4375;
	...
	private long lengthInBaseUnit(int length) {
		return length * getLength();
	}

	private long earliest(long lengthInBaseUnit) {
		return lengthInBaseUnit;
	}

	public long earliestFromLength(int length) {
		return earliest(lengthInBaseUnit(length));
	}

	private long latest(long lengthInBaseUnit) {
		return lengthInBaseUnit + getLength() - 1;
	}

	public long latestFromLength(int length) {
		return latest(lengthInBaseUnit(length));
	}
